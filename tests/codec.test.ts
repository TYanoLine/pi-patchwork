...
describe('pipw codec',()=>{
  ...
  it('streams progress and preview patches',()=>{const phases:string[]=[],previews:number[]=[];encode(sample(32,32),digits,index,20,0,50,4,'quality',70,90,{shouldPreview:()=>true,onProgress:(p)=>{phases.push(p.phase);previews.push(p.preview?.length??0);}});expect(phases).toContain('roots');expect(phases).toContain('final');expect(previews.some((n)=>n>0)).toBe(true);});
  ...
  it('flushes throttled preview updates before finishing',()=>{let finalPreview=0;encode(sample(32,32),digits,index,20,0,50,4,'quality',70,90,{shouldPreview:()=>false,onProgress:(p)=>{if(p.phase==='final')finalPreview=Math.max(finalPreview,p.preview?.length??0);}});expect(finalPreview).toBeGreaterThan(0);});
  ...
  it('keeps adaptive patch geometry readable in a mixed-detail image',()=>{const src=sample(128,128);for(let y=0;y<128;y++)for(let x=0;x<128;x++){const p=(y*128+x)*4,v=x<64?x*2:(x*31+y*17)%200;src.data[p]=v;src.data[p+1]=v;src.data[p+2]=v;}const r=encode(src,digits,index,10,0),sizes=new Set(patchRects(r.bytes).map(([, ,w,h])=>Math.max(w,h)));expect(sizes.size).toBeGreaterThanOrEqual(1);expect(patchRects(r.bytes).length).toBe(r.stats.patches);expect(r.bytes.length).toBeLessThanOrEqual(r.stats.budgetBytes);});
