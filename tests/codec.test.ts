...
describe('pipw codec',()=>{
  ...
  it('streams progress and preview patches',()=>{const phases:string[]=[],previews:number[]=[];encode(sample(32,32),digits,index,20,0,50,4,{shouldPreview:()=>true,onProgress:(p)=>{phases.push(p.phase);previews.push(p.preview?.length??0);}});expect(phases).toContain('roots');expect(phases).toContain('final');expect(previews.some((n)=>n>0)).toBe(true);});
  ...
  it('flushes throttled preview updates before finishing',()=>{let finalPreview=0;encode(sample(32,32),digits,index,20,0,50,4,{shouldPreview:()=>false,onProgress:(p)=>{if(p.phase==='final')finalPreview=Math.max(finalPreview,p.preview?.length??0);}});expect(finalPreview).toBeGreaterThan(0);});
