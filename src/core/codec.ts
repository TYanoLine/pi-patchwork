...
type Record = { offset:number; bias:[number,number,number]; gain:[number,number,number]; transform:number; repeat:number; phase:number; solid:boolean };
...
function reconstructionError(r:Record,data:Uint8ClampedArray,width:number,x0:number,y0:number,tw:number,th:number,d:Uint8Array){
  let total=0;for(let y=0;y<th;y++)for(let x=0;x<tw;x++){const q=r.solid?0:qAt(d,r.offset,x,y,r.transform,r.repeat,r.phase,tw,th),p=((y0+y)*width+x0+x)*4,edge=x===0||y===0||x===tw-1||y===th-1?1.35:1;for(let ch=0;ch<3;ch++){const delta=data[p+ch]-clamp(r.bias[ch]+r.gain[ch]*q);total+=delta*delta*edge;}}return total;
}
...
function best(data:Uint8ClampedArray,width:number,x0:number,y0:number,tw:number,th:number,d:Uint8Array,quality:number){
  let out=solid(data,width,x0,y0,tw,th),bestError=reconstructionError(out,data,width,x0,y0,tw,th,d);for(const candidate of shortlist(data,width,x0,y0,tw,th,d,quality)){const record=fit(data,width,x0,y0,tw,th,d,candidate),error=reconstructionError(record,data,width,x0,y0,tw,th,d);if(error<bestError){bestError=error;out=record;}}return out;
}
export function encode(source:ImageData,digits:Uint8Array,savePercent:number,quality=1):EncodeResult {
  const raw=source.width*source.height*3,budget=Math.max(HEADER_BYTES+RECORD_BYTES,Math.floor(raw*savePercent/100)),maxRecords=Math.max(1,Math.floor((budget-HEADER_BYTES)/RECORD_BYTES));
  const tile=Math.max(4,Math.ceil(Math.sqrt(source.width*source.height/maxRecords))),cols=Math.ceil(source.width/tile),rows=Math.ceil(source.height/tile),records:Record[]=[];
  for(let gy=0;gy<rows;gy++)for(let gx=0;gx<cols;gx++){const x=gx*tile,y=gy*tile,tw=Math.min(tile,source.width-x),th=Math.min(tile,source.height-y);records.push(best(source.data,source.width,x,y,tw,th,digits,quality));}
  const bytes=new Uint8Array(HEADER_BYTES+records.length*RECORD_BYTES),view=new DataView(bytes.buffer);MAGIC.forEach((m,i)=>view.setUint8(i,m));view.setUint8(4,2);view.setUint16(5,source.width,true);view.setUint16(7,source.height,true);view.setUint16(9,tile,true);view.setUint16(11,cols,true);view.setUint16(13,rows,true);view.setUint32(15,digits.length,true);view.setUint32(19,records.length,true);
  records.forEach((r,i)=>{const p=HEADER_BYTES+i*RECORD_BYTES;view.setUint32(p,r.offset,true);for(let ch=0;ch<3;ch++)view.setUint8(p+4+ch,r.bias[ch]);for(let ch=0;ch<3;ch++)view.setInt8(p+7+ch,r.gain[ch]);view.setUint8(p+10,r.transform|(r.repeat<<3)|(r.phase<<5));view.setUint8(p+11,r.solid?1:0);});
...
export function decode(bytes:Uint8Array,digits:Uint8Array):ImageData {
  const view=new DataView(bytes.buffer,bytes.byteOffset,bytes.byteLength),version=bytes.length>=5?view.getUint8(4):0;if(bytes.length<HEADER_BYTES||MAGIC.some((m,i)=>view.getUint8(i)!==m)||(version!==1&&version!==2))throw new Error('対応していない .pipw です');
  const w=view.getUint16(5,true),h=view.getUint16(7,true),tile=view.getUint16(9,true),cols=view.getUint16(11,true),rows=view.getUint16(13,true),need=view.getUint32(15,true),count=view.getUint32(19,true);if(!w||!h||w*h>16_777_216||digits.length<need||count!==cols*rows||HEADER_BYTES+count*RECORD_BYTES!==bytes.length)throw new Error('破損または辞書が一致しません');
