...
export default function App(){
  ...
  async function pick(file?:File){if(!file)return;setError('');try{setSource(await fileToImageData(file));setResult(undefined);}catch{setError('画像を読み込めませんでした');}}
  ...
  return <main>
    ...
    <section className="workbench"><aside>
      ...
      {result&&<><div className="stats"><div><small>実サイズ</small><strong>{result.stats.actualBytes.toLocaleString()} B</strong></div><div><small>保存率</small><strong>{result.stats.ratio?result.stats.ratio.toFixed(2)+'%':'—'}</strong></div><div><small>PSNR</small><strong>{result.stats.psnr?result.stats.psnr.toFixed(2)+' dB':'—'}</strong></div><div><small>パッチ</small><strong>{result.stats.patches}</strong></div><div><small>πパターン</small><strong>{result.stats.piPatches||'—'}</strong></div></div><button className="download" onClick={save}><Download/> .pipw を保存</button></>}
    </div></section>
    <section className="how" id="how"><span>HOW IT WORKS</span><h2>画像ではなく、<em>作り方</em>を保存する。</h2><ol><li><b>01</b><strong>分割</strong><p>サイズ予算に合うパッチへ分けます。</p></li><li><b>02</b><strong>探索</strong><p>視覚的に近い桁の模様を探します。</p></li><li><b>03</b><strong>補正</strong><p>色・回転・反転・反復を調整します。</p></li><li><b>04</b><strong>再構成</strong><p>参照と補正値から描き直します。</p></li></ol></section>
