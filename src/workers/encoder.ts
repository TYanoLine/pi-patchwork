...
self.onmessage = (event: MessageEvent<{image: ImageData; digits: string; index: ArrayBuffer; savePercent: number; quality: number}>) => {
  try {
    const digits = parseDigits(event.data.digits);
    const index = parsePiIndex(event.data.index);
    const result = encode(event.data.image, digits, index, event.data.savePercent, event.data.quality);
